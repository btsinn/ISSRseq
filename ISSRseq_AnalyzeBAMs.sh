...
0.5 -- 	removed fastStructure
	added minimum base quality for HaplotypeCaller variant scoring
  added directory variants/haplotypecallerBAMs which contains BAM files generated by HaplotypeCaller during read realignment, from which variants were called
...
do
    ...
    gatk --java-options "-Xmx100g" HaplotypeCaller --linked-de-bruijn-graph --native-pair-hmm-threads $THREADS --native-pair-hmm-use-double-precision true -R $REF_DB -L $PREFIX/reference/list.intervals -I $PREFIX/bams/${sample}_sorted_RG.bam -O $PREFIX/gvcfs/${sample}.g.vcf -ERC GVCF -bamout $PREFIX/variants/haplotypecallerBAMs/${sample}_HaplotypeCaller_out.bam >>$PREFIX/ISSRseq_AnalyzeBAMs.log 2>&1
